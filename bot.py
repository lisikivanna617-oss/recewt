...
async def check_single_username(session: aiohttp.ClientSession, username: str) -> bool:
    ...
    try:
        async with session.get(url, headers=headers, allow_redirects=True) as resp:
            ...
            # Надійні маркери вільного юзернейму в Telegram веб-інтерфейсі
            if any(marker in text for marker in [
                ...
                # Додаткова перевірка: якщо сторінка містить шаблон профілю каналу/юзера з ім'ям, він зайнятий
...
async def generate_and_find_free(user_id: int, prefix: str = "", suffix: str = "", target_length: int = 6, use_digits: bool = True, digits_count: int = 1) -> str:
    """Швидкий генератор з використанням випадкових літер та гарантованим знаходженням унікальних комбінацій"""
    letters = "abcdefghijklmnopqrstuvwxyz"
    digits = "0123456789"
    
    timeout = aiohttp.ClientTimeout(total=1.5)
    async with aiohttp.ClientSession(timeout=timeout) as session:
        # Генеруємо набір розумних кандидатів замість безкінечного сліпого пошуку
        candidates = set()
        
        if prefix:
            prefix = prefix.lstrip("@").lower()
            # Додаємо варіанти з суфіксами або цифрами під префікс
            ...
            # Автоматичний пошук: комбінації з рандомних літер + складніших шаблонів
            for _ in range(30):
        ...
        # Швидко перевіряємо сформований пул кандидатів
